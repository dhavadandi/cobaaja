...
// 8-Year Baseline Annual Dataset (Ton)
...
export function DashboardProvider({ children }: { children: React.ReactNode }) {
  ...
  const setSelectedRegency = (regency: string) => {
    ...
    if (regObj && !regObj.target && regency !== "all") {
      showToast(`Filter: ${name} (Wilayah Luar Fokus 5 Sentra Utama - Data Historis BPS Tersedia)`);
    } else {
  ...
  const setSelectedMonth = (monthKey: string) => {
    ...
    let label = "Seluruh 36 Bulan Proyeksi (2026–2028)";
    if (monthKey.startsWith("hist-")) {
      label = `Tahun Historis ${monthKey.replace("hist-", "")} (Data Aktual BPS)`;
    } else if (monthKey.startsWith("yr-")) {
      label = `Tahun Proyeksi ${monthKey.replace("yr-", "")} (12 Bulan)`;
  ...
  const resetFilters = () => {
    ...
    showToast("Seluruh Filter Berhasil Di-reset ke Default (Mode Proyeksi)");
  };
